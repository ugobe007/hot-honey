#!/usr/bin/env node
/**
 * Improved funding extraction with better duplicate detection
 */
...
async function extractFunding() {
  console.log('🔍 Extracting funding (improved duplicate detection)...\n');
  ...
  for (const article of articles) {
    try {
      ...
      let date = article.published_at?.split('T')[0] || new Date().toISOString().split('T')[0];
  ...
  console.log(`\n📈 Summary: ${extracted} extracted, ${skipped} skipped, ${duplicates} duplicates avoided`);
}
