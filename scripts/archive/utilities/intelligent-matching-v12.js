#!/usr/bin/env node
/**
 * INTELLIGENT MATCHING v12
 * ========================
 * 
 * NEW: Velocity Score - rewards fast-moving startups
 * - Speed to MVP
 * - Speed to revenue  
...
/**
 * VELOCITY SCORE (0-20)
 * Measures how fast the startup moves
 */
function calculateVelocityScore(startup) {
  ...
  if (daysToMvp !== null && daysToMvp !== undefined) {
    if (daysToMvp <= 30) { velocity += 5; signals.push('mvp<30d'); }
    else if (daysToMvp <= 60) { velocity += 4; signals.push('mvp<60d'); }
    else if (daysToMvp <= 90) { velocity += 3; signals.push('mvp<90d'); }
    else if (daysToMvp <= 180) { velocity += 2; signals.push('mvp<6mo'); }
    else { velocity += 1; }
  }
  
  // 2. Time to first revenue (0-4)
  ...
  if (monthsToRevenue !== null && monthsToRevenue !== undefined && monthsToRevenue > 0) {
    if (monthsToRevenue <= 3) { velocity += 4; signals.push('rev<3mo'); }
    else if (monthsToRevenue <= 6) { velocity += 3; signals.push('rev<6mo'); }
    else if (monthsToRevenue <= 12) { velocity += 2; signals.push('rev<12mo'); }
    else { velocity += 1; }
  }
  
  // 3. Deployment frequency (0-3)
  const deployFreq = startup.deployment_frequency;
  if (deployFreq === 'daily' || deployFreq === 'continuous') { velocity += 3; signals.push('deploy:daily'); }
  else if (deployFreq === 'weekly') { velocity += 2; signals.push('deploy:weekly'); }
  else if (deployFreq === 'monthly') { velocity += 1; }
  
  // 4. Growth rate (0-4)
  const growthRate = startup.growth_rate_monthly || startup.arr_growth_rate || 0;
  if (growthRate >= 30) { velocity += 4; signals.push('growth>30%'); }
  else if (growthRate >= 20) { velocity += 3; signals.push('growth>20%'); }
  else if (growthRate >= 10) { velocity += 2; signals.push('growth>10%'); }
  else if (growthRate >= 5) { velocity += 1; }
  
  // 5. Pivot speed (0-2)
  const pivotSpeed = startup.pivot_speed_days;
  if (pivotSpeed !== null && pivotSpeed !== undefined && pivotSpeed > 0) {
    if (pivotSpeed <= 14) { velocity += 2; signals.push('pivot<2wk'); }
    else if (pivotSpeed <= 30) { velocity += 1; signals.push('pivot<1mo'); }
  }
  
  // 6. Features shipped (0-2)
  const features = startup.features_shipped_last_month || 0;
  if (features >= 10) { velocity += 2; signals.push('ship:10+'); }
  else if (features >= 5) { velocity += 1; signals.push('ship:5+'); }
  
  return { score: Math.min(20, velocity), signals };
}
...
async function runTest() {
  console.log('Testing v12 (with VELOCITY scoring)...');
  ...
  // Show velocity breakdown for first 5 startups
  console.log('\nVelocity samples:');
  for (const s of startups.slice(0, 5)) {
    const v = calculateVelocityScore(s);
    console.log('  ' + s.name.substring(0, 25).padEnd(27) + ' velocity: ' + v.score + '/20  [' + v.signals.join(', ') + ']');
  }
...
async function main() {
  ...
  else console.log('Usage: node intelligent-matching-v12.js --test');
}
