...
function calculateSectorScore(startup, investor) {
  ...
  // Check for exact match - MODERATE reward (be skeptical)
  for (const ss of startupSectors) {
    if (investorSectors.includes(ss)) {
      return { score: 25, reason: 'exact_match', match: ss };
    }
  }
  
  // Check for adjacent sectors - SMALL reward
  for (const ss of startupSectors) {
    const adjacent = SECTOR_ADJACENCY[ss] || [];
    for (const is of investorSectors) {
      if (adjacent.includes(is)) {
        return { score: 10, reason: 'adjacent', startup: ss, investor: is };
      }
    }
  }
...
function calculateStageScore(startup, investor) {
  ...
  // Exact stage match
  if (investorStageNums.includes(startupStage)) {
    return { score: 15, reason: 'exact_stage' };
  }
  
  // Startup ready for next round (investor invests at next stage)
  if (investorStageNums.includes(startupStage + 1)) {
    return { score: 10, reason: 'ready_for_next' };
  }
  
  // One stage apart
  const minDiff = Math.min(...investorStageNums.map(n => Math.abs(n - startupStage)));
  if (minDiff === 1) {
    return { score: 5, reason: 'one_stage_off' };
  }
  
  // Two stages apart - PENALTY
  if (minDiff === 2) {
    return { score: 0, reason: 'two_stages_off' };
  }
...
function calculateGodScoreBonus(startup) {
  ...
  // Be skeptical - only reward truly exceptional GOD scores
  if (godScore >= 70) return { score: 12, tier: 'exceptional' };
  if (godScore >= 60) return { score: 8, tier: 'strong' };
  if (godScore >= 50) return { score: 5, tier: 'good' };
  if (godScore >= 40) return { score: 2, tier: 'moderate' };
  return { score: 0, tier: 'weak' };
}
...
function calculateMatchScore(startup, investor) {
  ...
  totalScore += stageResult.score;     // 0-25
