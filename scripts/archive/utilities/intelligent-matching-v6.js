#!/usr/bin/env node
/**
 * INTELLIGENT MATCHING v6
 * =======================
 * 
 * Changes from v5:
 * 1. Increased bonuses for exact matches (spread the top)
 * 2. Stronger penalties for confirmed mismatches (spread the bottom)
 * 3. Removed sigmoid compression (let natural variance show)
 * 4. Base score 20 to center distribution
 */
...
function calculateMatchScore(startup, investor) {
  // BASE SCORE: 20 (center of distribution)
  let score = 20;
  ...
  if (startupSectors.length === 0 || investorSectors.length === 0) {
    // Unknown - small boost (could match anything)
    score += 3;
    ...
    if (exactMatch) {
      score += 25;  // Strong bonus for exact match
      ...
      if (adjacent) {
        score += 12;  // Good bonus for adjacent
        sectorReason = 'adjacent';
      } else {
        score -= 12;  // Strong penalty for mismatch
...
async function runTest() {
  ...
  console.log('🧪 TESTING INTELLIGENT MATCHING v6');
  console.log('═'.repeat(60));
  console.log('\nKey changes: Stronger bonuses/penalties, no compression\n');
...
async function main() {
  ...
  else console.log('Usage: node intelligent-matching-v6.js --test');
}
