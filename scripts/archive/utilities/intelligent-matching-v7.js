#!/usr/bin/env node
/**
 * INTELLIGENT MATCHING v7
 * =======================
 * 
 * Changes from v6:
 * 1. Wider spread - bigger bonuses AND bigger penalties
 * 2. Lower base score (15) to allow more room at bottom
 * 3. Double mismatch = very low score
 */
...
function calculateMatchScore(startup, investor) {
  // BASE SCORE: 15 (lower to allow spread)
  let score = 15;
  ...
  if (startupSectors.length === 0 || investorSectors.length === 0) {
    score += 8;  // Unknown gets moderate boost
    ...
    if (exactMatch) {
      score += 30;  // BIG bonus for exact match
      ...
      if (adjacent) {
        score += 15;  // Good bonus for adjacent
        sectorReason = 'adjacent';
      } else {
        score -= 15;  // BIG penalty for mismatch
  ...
  // DOUBLE MISMATCH PENALTY
  if (sectorMismatch && stageMismatch) {
    score -= 5;  // Extra penalty for being wrong on both
  }
  ...
  if (raiseAmount > 0 && maxCheck < Infinity) {
    ...
    if (minCheck >= idealMin * 0.5 && maxCheck <= idealMax * 2) score += 8;
  ...
  else if (sectorMismatch || stageMismatch) confidence = 'low';
...
async function runTest() {
  ...
  console.log('🧪 TESTING INTELLIGENT MATCHING v7');
  console.log('═'.repeat(60));
  console.log('\nKey changes: Wider spread, double mismatch penalty\n');
  ...
  for (const startup of startups) {
    ...
    for (const investor of sampleInvestors) {
      ...
      console.log('   → ' + name.padEnd(27) + ' Score: ' + result.score.toString().padStart(2) + ' | ' + result.breakdown.sector + ', ' + result.breakdown.stage);
    }
  }
  ...
  const buckets = { '0-20': 0, '21-35': 0, '36-50': 0, '51-65': 0, '66-85': 0 };
  let total = 0, sum = 0;
  ...
  console.log('\nTarget: 0-20: ~5%, 21-35: ~15%, 36-50: ~35%, 51-65: ~30%, 66-85: ~15%');
}

async function main() {
  const args = process.argv.slice(2);
  if (args.includes('--test')) await runTest();
  else console.log('Usage: node intelligent-matching-v7.js --test');
}
