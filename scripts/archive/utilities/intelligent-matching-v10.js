...
function calculateMatchScore(startup, investor) {
  ...
  if (startupSectors.length === 0 || investorSectors.length === 0) {
    ...
    if (exactMatch) {
      ...
      if (adjacent) {
        ...
        score += 0;  // mismatch = no bonus, not penalty
        sectorReason = 'no_match';
      }
    }
  }
  
  // STAGE: 0 to 18 (no negatives)
  ...
  if (startupStage === null || startupStage === undefined) {
    ...
    if (minDiff === 1) {
      ...
      score += 0;  // far off = no bonus
      stageReason = 'stage_mismatch';
    }
  }
  
  // TRACTION: 0-8
  if (startup.has_revenue) score += 8;
  else if (startup.has_customers) score += 5;
  else if (startup.is_launched) score += 2;
  
  // GOD: 0-6
  const godScore = startup.total_god_score || 0;
  if (godScore >= 65) score += 6;
  else if (godScore >= 55) score += 4;
  else if (godScore >= 45) score += 3;
  else if (godScore >= 35) score += 1;
  ...
  if (raiseAmount > 0 && maxCheck < Infinity) {
    ...
    if (minCheck >= idealMin * 0.5 && maxCheck <= idealMax * 2) score += 5;
    else if (maxCheck >= raiseAmount * 0.05 && minCheck <= raiseAmount * 0.5) score += 3;
  } else {
    score += 2;
  }
  
  // Score range: 30 (base) + 0-57 (bonuses) = 30-87
  // Normalize to 15-80 range
  const normalized = 15 + ((score - 30) / 57) * 65;
  const finalScore = Math.min(80, Math.max(15, Math.round(normalized)));
...
async function runTest() {
  console.log('Testing v10 (no penalties, normalized range)...');
...
async function main() {
  ...
  else console.log('Usage: node intelligent-matching-v10.js --test');
}
