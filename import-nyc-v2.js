#!/usr/bin/env node
/**
 * Import NYC Founder Guide Investors v2
 * Uses direct Supabase client instead of exec_sql
 */
...
async function importInvestors() {
  console.log('\n╔══════════════════════════════════════════════════════════════════════════╗');
  console.log('║           🗽 NYC FOUNDER GUIDE INVESTOR IMPORT v2                         ║');
  console.log('╚══════════════════════════════════════════════════════════════════════════╝\n');
  ...
  for (let i = 0; i < nycInvestors.length; i++) {
    ...
    // Check if exists using direct query
    ...
    if (existing) {
      // Update existing - only use columns that are in the cache
      ...
      if (error) {
        console.log(`❌ [${i+1}] ${inv.firm} - update failed: ${error.message}`);
        errors++;
      } else {
      ...
      // Insert new - only use columns that are in the cache
      const { error } = await supabase
        .from('investors')
        .insert(investorRecord);
      
      if (error) {
        console.log(`❌ [${i+1}] ${inv.firm} - insert failed: ${error.message}`);
        errors++;
      } else {
  ...
  console.log(`   🏢 Total investors in DB: ${count}\n`);
}
