-- Check if pattern_based_extraction startups are actually in startup_uploads
...
SELECT 
  ...
  COUNT(*) FILTER (WHERE ds.imported_to_startups = false OR ds.imported_to_startups IS NULL)::bigint as need_import,
  COUNT(*) FILTER (WHERE ds.imported_to_startups = true AND su.id IS NULL)::bigint as marked_but_not_in_table,
