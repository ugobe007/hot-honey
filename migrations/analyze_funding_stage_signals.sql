-- Analyze Funding Stage Signals - Find Missing Series A/B Data
...
-- ============================================================
-- 2. Check Funding Round Data in funding_rounds Table
-- ============================================================
-- Only run if funding_rounds table exists
...
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE 'funding_rounds table exists';
  ELSE
    RAISE NOTICE 'funding_rounds table does NOT exist - skipping funding_rounds queries';
  END IF;
END $$;

-- Check funding_rounds only if it exists
SELECT 
  'FUNDING ROUNDS TABLE' as section,
  COUNT(*)::bigint as total_rounds,
  COUNT(DISTINCT startup_id)::bigint as startups_with_rounds,
  round_type,
...
SELECT 
  'RSS ARTICLES WITH FUNDING MENTIONS' as section,
...
-- ============================================================
-- First check if funding_rounds exists and what columns it has

SELECT 
  'POTENTIAL SERIES A/B STARTUPS' as section,
  su.id,
  su.name,
  su.stage,
  su.raise_type,
  ...
  CASE 
    WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'funding_rounds')
    THEN (SELECT fr.round_type FROM funding_rounds fr WHERE fr.startup_id = su.id AND (fr.round_type ILIKE '%series a%' OR fr.round_type ILIKE '%series b%') LIMIT 1)
    ELSE NULL
  END as funding_round_type
FROM startup_uploads su
WHERE (
  -- Has funding amount but stage not set
  (su.raise_amount IS NOT NULL AND (su.stage IS NULL OR su.stage < 2))
  -- Or extracted data suggests funding but stage is wrong
  OR (su.extracted_data->>'funding_amount' IS NOT NULL AND (su.stage IS NULL OR su.stage < 2))
  -- Or raise_type mentions Series A/B but stage is wrong
  OR (su.raise_type ILIKE '%Series A%' AND (su.stage IS NULL OR su.stage != 2))
  OR (su.raise_type ILIKE '%Series B%' AND (su.stage IS NULL OR su.stage != 3))
  -- Or extracted_data has Series A/B but stage is wrong
...
SELECT 
  'FUNDING SIGNAL PATTERNS IN ARTICLES' as section,
  CASE 
