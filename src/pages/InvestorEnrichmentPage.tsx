  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a1140] via-[#2d1b69] to-[#4a2a8f] relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-8 py-12">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-5xl font-bold text-white mb-3">
                Investor <span className="bg-gradient-to-r from-orange-400 to-amber-400 bg-clip-text text-transparent">Enrichment</span>
              </h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={loadData}
                disabled={loading}
                className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 disabled:bg-gray-600 text-white font-bold rounded-lg transition-all shadow-lg"
              >
        ...
        {/* Stats Grid */}
        <div className="bg-gradient-to-br from-orange-900/80 to-amber-900/80 backdrop-blur-lg border-2 border-orange-400/60 rounded-3xl p-8 shadow-2xl mb-12">
        ...
        {/* Data Completeness */}
        <div className="bg-gradient-to-br from-amber-900/80 to-yellow-900/80 backdrop-blur-lg border-2 border-amber-400/60 rounded-3xl p-8 shadow-2xl mb-12">
          <div className="flex items-center gap-3 mb-6">
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="bg-white/5 rounded-xl p-6 border border-white/10">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-orange-400 font-bold text-lg">{Math.round((stats.hasPartners / stats.total) * 100)}%</span>
              </div>
            ...
            <div className="bg-white/5 rounded-xl p-6 border border-white/10">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-amber-400 font-bold text-lg">{Math.round((stats.hasInvestments / stats.total) * 100)}%</span>
              </div>
            ...
            <div className="bg-white/5 rounded-xl p-6 border border-white/10">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-amber-400 font-bold text-lg">{Math.round((stats.hasThesis / stats.total) * 100)}%</span>
              </div>
            ...
            {recentInvestors.map((investor, i) => (
              <div key={investor.id} className="bg-white/5 rounded-xl p-4 border border-white/10 flex items-center justify-between hover:bg-white/10 transition-all">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-orange-500/20 to-amber-500/20 border border-orange-400/50 flex items-center justify-center text-orange-300 font-bold">
                    {i + 1}
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-3">
                  {investor.partners ? (
                    ...
                    <div className="px-4 py-2 bg-amber-500/20 border-2 border-amber-400 rounded-lg text-amber-300 font-bold text-sm uppercase">
                      ⏳ Pending
                    </div>
                  )}
                  <button
                    onClick={() => navigate(`/investor/${investor.id}`)}
                    className="px-6 py-2 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-bold rounded-lg transition-all shadow-lg"
                  >
                    View →
                  </button>
                </div>
