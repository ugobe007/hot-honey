  ...
  return (\
    <div className="min-h-screen bg-gradient-to-br from-gray-300 to-gray-400">\
      <div className="container mx-auto p-4">\
        <h1 className="text-3xl font-bold mb-6 text-center text-gray-800">Hot Money: Startup Deal Review</h1>\
        <ScrollArea className="h-[calc(100vh-100px)]">\
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">\
            \{startupDeals.map((deal) => (\
              <Card key=\{deal.id\} className="flex flex-col bg-white shadow-lg">\
                <CardHeader className="pb-2">\
                  <div className="flex justify-between items-center">\
                    <CardTitle className="text-xl">\{deal.name\}</CardTitle>\
                    \{isHotMoney(deal.id) && (\
                      <Button className="bg-orange-500 hover:bg-orange-600 text-white font-bold py-1 px-2 rounded flex items-center gap-1">\
                ...
                <CardContent className="flex-grow pt-2">\
                  <div className="space-y-3">\
                    ...
                    <div>\
                      ...
                      <p className="text-sm">\{deal.problem\}</p>\
                    </div>\
                    <div>\
                      <h3 className="font-semibold mb-1">Solution:</h3>\
                      <p className="text-sm">\{deal.solution\}</p>\
                    </div>\
                    <div>\
